...
**[Region drag UI — NSPanel overlay + NSView vs SwiftUI gesture]**: 사용자가 *영역 그림* UI 옵션 = (a) SwiftUI DragGesture (단 full-screen overlay + key event handler 박는 거 복잡) / (b) **NSPanel borderless + NSView (mouseDown/Dragged/Up + keyDown)** / (c) Carbon CGEventTap (사용자 입력 전역 박음 — overkill). 선택 (b) — AppKit 박은 거 *NSWindow 박힌 거*가 macOS native, full-screen cover + 반투명 background + canBecomeKey=true (ESC handler). `RegionEditorPanel.canBecomeKey override` (NSPanel default false). NSView가 *mouseDown*시 dragStart 박음 + *mouseDragged*시 dragCurrent 박음 + *mouseUp*시 NSRect 박은 후 PrivacySettings.addSensitiveRegion. ESC/Return = 종료, Delete = 마지막 영역 제거. 박힌 영역 빨강 (alpha 0.5) + 라벨 #1/#2, 드래그 중 주황 (alpha 0.35). multi-monitor → cursor 있는 display만 박음. *되돌리기 비용*: RegionEditorWindow.swift 삭제 + SettingsView regionsSection 박은 거 + AppDelegate menu item + @objc 호출 — 15분.
