...
enum Prompts {
    ...
    `coordinates`는 선택. 화면에 텍스트가 없는 아이콘/이미지 메뉴를 가리킬 때만. 평소엔 키 자체를 생략.
    ...
    화면에 visible text가 정말 없을 때만 `[x, y, w, h]` 정수 배열. 좌표는 사용자가 본 이미지의 픽셀 기준. 좌상단이 `(0, 0)`. 평소엔 키 자체를 생략 — backend OCR가 `target_text`로 위치를 정확히 찾는다.
    """
}
